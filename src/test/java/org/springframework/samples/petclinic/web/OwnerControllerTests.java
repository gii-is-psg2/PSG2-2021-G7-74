...
class OwnerControllerTests {
	...
	void testShowOwner() throws Exception {
		given(this.clinicService.getLoggedOwner()).willReturn(george);
	...
	void testDeleteOwner() throws Exception {
		mockMvc.perform(get("/owners/delete/{ownerId}", TEST_OWNER_ID).with(csrf()))
				.andExpect(status().is3xxRedirection()).andExpect(view().name("redirect:/owners?lastName="));
	}
}
